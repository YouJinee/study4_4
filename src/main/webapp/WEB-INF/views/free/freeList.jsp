			...
			<div class="col-sm-3 form-inline">
				...
				<select id="id_rowSizePerPage" name="rowSizePerPage" class="form-control input-sm">
					<c:forEach var="i" begin="10" end="50" step="10">
						<option value="${i }" ${searchVO.rowSizePerPage eq i ? "selected='selected'" : "" }>${i }
						</option>
					</c:forEach>
				</select>
			</div>
...
<script type="text/javascript">
		...
		$('ul.pagination li a[data-page]').click(function(e) {
			// a링크를 누르면 get 방식으로 감 => 파라미터까 curPage 한개만 넘어간다. ]
			//searchVO에 해당하는 데이터 (search *, rowSizePerPage 등 +curPage)== form 태그
			//데이터가 같이 넘어가도록 get 방식 대신에 form 태그 post submit 하도록 변경
			//검색도 했고 현재 5번글이다. => 8번 글을 눌렀다
			//검색과 rowSizePerPage가 유지되면서 curPage만 8로 바뀌면 된다.
			
			//a링크 기본 이벤트를 preventDefault
			//form태그 밑에 input type hidden name = curPage 태그에 value 값을 현재 a링크 data-page의 값으로 변경해줌
			//form 태그 submit 
			//data 속성 : data를 담기 위한 속성이다.
			
			//<a href="#" data-aaaa="30"  data-bbbb="25" data-ccc="0"...... id = "myA">
	 		/* $("#myA").data("aaaa"); ==30
			$("#myA").data("bbbb"); ==25
			$("#myA").data("ccc",5); ==data-ccc=5로 변경
			 */
			e.preventDefault(); 
			$curPage.val($(this).data("page"));
			$form.submit();
			//기본 이벤트 막기, curPage변경, ( 현재 내가 누른 a태그 data-page에 있어요), 
			//form, submit
